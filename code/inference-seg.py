...
#     original_image = input_object.cpu().numpy()
...
#          "sematic_seg":instance["instances"].pred_masks.cpu().numpy().tolist() #this stalls the program and crashes for large images
#         }
#     # add to python list/array
#     instances.append(x)
    masklist = []
    for maskX in instance['instances'].pred_masks:
        mask=np.array((maskX.cpu() == True),  dtype=int)
        masklist.append(mask.tolist())
    # a Python object (dict):
    x = {"pred_boxes":instance['instances'].pred_boxes.tensor.cpu().numpy().tolist(),
        "pred_classes":instance['instances'].pred_classes.cpu().numpy().tolist(),
         "scores":instance['instances'].scores.cpu().numpy().tolist(),
         "sematic_seg":masklist # instance["instances"].pred_masks.cpu().numpy().tolist() #this stalls the program and crashes for large images
        }
    # add to python list/array
    instances.append(x)  
        # convert into JSON:
